...
import com.openbravo.pos.bespoke.RetourInfo;
...
*/                
    ...
    public final SentenceExec getCustomerDelete() {
     ...
     public final RetourInfo getRetour() throws BasicException {
        return (RetourInfo) new PreparedSentence(s, "SELECT IDTICKET FROM RETOUR WHERE 'a'=?", SerializerWriteString.INSTANCE, RetourInfo.getSerializerRead()).find("a");
    }

    public void setRetour(int id) {
        final int _id = id;

        try {

            new StaticSentence(s, "DELETE FROM RETOUR", SerializerWriteString.INSTANCE).exec();
            SentenceExec insertretour = new PreparedSentence(s, "INSERT INTO RETOUR(IDTICKET) VALUES (?)", SerializerWriteParams.INSTANCE);
            insertretour.exec(new DataParams() {
